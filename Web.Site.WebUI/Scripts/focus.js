...
function focus_show(id)
{
	if(focus_time < 20 && focus_bg != "")
	{
		...
		if(!focus_begin)
		{
		    ...
		    document.getElementById("focus_bg").innerHTML = document.getElementById("focus_" + focus_bg).innerHTML;
			focus_time = 0;
		...
		//document.getElementById("focus_show").innerHTML = "<a href='" + focus_url + "' target='_blank' title='" + focus_title + "'>" + document.getElementById("focus_" + focus_id).innerHTML + "</a>";
		document.getElementById("focus_show").innerHTML = document.getElementById("focus_" + focus_id).innerHTML;
		focus_url = arr[1];
		focus_title = arr[2];
		if (!focus_begin) focus_interval = setInterval("func_focus()", 6000); else focus_begin = false;
	    //focus_bg = arr[0];
		focus_bg = focus_id;
