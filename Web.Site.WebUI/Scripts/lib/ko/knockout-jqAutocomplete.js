; (function (factory) {
    ...
    var JqAuto = function () {
        ...
        this.init = function (element, valueAccessor, allBindings, data, context) {
            ...
            widget.menu.element.addClass("contact-book");
            if (!options.isFloor) {
                widget.menu.element.addClass("on-fancybox");
            }
            if (options.zIndexClass) {
                widget.menu.element.addClass("zmax");
            }
            if (options.isSpecial) {
                widget.menu.element.addClass("list-group");
            }
        ...
        this.processOptions = function (valueAccessor, filter, data, request, response) {
            ...
            var isObservable = ko.isObservable(items);
            var itemsLength = 0;
            if (isObservable) {
            ...
            //filter/map items
            for (index = 0, length = itemsLength; index < length; index++) {
                if (isObservable) {
                    item = items()[index];
                } else {
                    item = items[index];
            ...
            if (results.length < 1) {
                results.push({ label: self.options.emptyLable });
            }
        ...
        this.renderItem = function (templateName, context, ul, item) {
            if (item.label === self.options.emptyLable) {
                var emptyLi = $("<li><a class='list-group-item ui-corner-all'><p><span>" + self.options.emptyLable + "</span></p></a></li>").addClass("row").appendTo(ul);
                return emptyLi;
            }

            var $li = $("<li></li>").addClass("row").appendTo(ul),
        ...
        this.options = {
            autoFocus: true,
            delay: 50,
            emptyLable: 'No results found!',
            isSpecial: false,
            isFloor: false, // autocomplete 是否处于最底层
            zIndexClass: null
        };
    };
