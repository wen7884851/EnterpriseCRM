    ...
    public abstract class EFRepositoryBase<TEntity, TKey> : IRepository<TEntity, TKey> where TEntity : EntityBase<TKey>
    {
        ...
        protected UnitOfWorkContextBase EFContext
        {
            get
            {
                ...
                throw new DataAccessException($"数据仓储上下文对象类型不正确，应为UnitOfWorkContextBase，实际为 {UnitOfWork.GetType().Name}");
            }
        }

        /// <summary>
        ///     获取 当前实体的查询数据集(不带缓存的查询)
        /// </summary>
        public virtual IQueryable<TEntity> NoCahecEntities
        {
